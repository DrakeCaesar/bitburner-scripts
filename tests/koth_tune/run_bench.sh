...
find_exe() {
  local candidates=(
    "$BUILD_DIR/Release/koth_bench.exe"
    "$BUILD_DIR/Release/koth_bench"
    "$BUILD_DIR/koth_bench.exe"
    "$BUILD_DIR/koth_bench"
  )
...
build_bench() {
  configure_if_needed
  cmake --build "$BUILD_DIR" --config Release --target koth_bench
}
...
if [[ "$FORCE" == "1" ]] || needs_rebuild; then
  echo "Building koth_bench..."
  build_bench
else
  echo "koth_bench is up to date (use --force to rebuild)"
fi
