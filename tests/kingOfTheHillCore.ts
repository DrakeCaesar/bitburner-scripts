...
export function sampleAltitudeProfile(
  ...
  const pointByX = new Map<number, { x: number; altitude: number; nearZone: boolean }>()
...
export function generateAssignmentAt(
  seed: number,
  index: number,
  difficulty: number,
): { index: number; assignment: KingOfTheHillAssignment } {
  const i = index - 1
