...
export async function main(ns) {
    ...
    ns.print(
        "\nmoney         " +
            ...
            "%" +
            "\nmoney         " +
            (moneyMax * (1 - hackProc) * (1 - monT)).toFixed(0) +
            " <= " +
            moneyCur.toFixed(0) +
            " <= " +
            (moneyMax * (1 - hackProc) * (1 + monT)).toFixed(0) +
    ...
    while (needSetGrow || needSetHack) {
        ...
        if (moneyMax == moneyCur && securityMin == securityCur) {
            hackThreads = Math.floor(
                ns.hackAnalyzeThreads(target, moneyMax * hackProc)
            )
            ...
            for (
                hackWeakThreads = 1;
                ns.weakenAnalyze(hackWeakThreads) <= hackSecurity;
                hackWeakThreads *= 1.25
            );
            ...
            if (needSetGrow) {
                ...
                ns.print(
                    "\n" +
                        ...
                        ns.tFormat(hackTime) +
                        "\nhack Security " +
                        hackSecurity.toFixed(2) +
                        "\n" +
                        "\nweak Threads  " +
                        hackWeakThreads +
                        "\nweak Time     " +
                        ns.tFormat(hackWeakTime) +
                        "\n" +
                        "\nmoney         " +
                        ((moneyCur / moneyMax) * 100).toFixed(2) +
                        "%" +
                ...
                ns.print("RUN WEAK")
                let pid2 = hackWeakThreads
                    ? ns.run("/hacking/weaken.js", hackWeakThreads, target)
                    : pid1
                if (
                    ns.getRunningScript(pid1) == null ||
                    ns.getRunningScript(pid2) == null
                ) {
                    ns.kill(pid1)
                    ns.kill(pid2)
                ...
                await ns.sleep(
                    Math.max(hackTime, hackWeakThreads ? hackWeakTime : 0)
                )
                while (
                    ns.getRunningScript(pid1) != null ||
                    ns.getRunningScript(pid2) != null
            ...
            moneyCur >= moneyMax * (1 - hackProc) * (1 - monT) &&
            moneyCur <= moneyMax * (1 - hackProc) * (1 + monT) &&
            securityMin == securityCur
        ) {
            growThreads = Math.ceil(
                ns.growthAnalyze(target, 1 / (1 - growProc))
            )
            ...
            for (
                growWeakThreads = 1;
                ns.weakenAnalyze(growWeakThreads) <= growSecurity;
                growWeakThreads *= 1.25
            );
            ...
            if (needSetHack) {
                ...
                ns.print(
                    "\n" +
                        ...
                        ns.tFormat(growTime) +
                        "\ngrow Security " +
                        growSecurity.toFixed(2) +
                        "\n" +
                        "\nweak Threads  " +
                        growWeakThreads +
                        "\nweak Time     " +
                        ns.tFormat(growWeakTime) +
                        "\n" +
                        "\nmoney         " +
                        ((moneyCur / moneyMax) * 100).toFixed(2) +
                        "%" +
                ...
                ns.print("RUN WEAK")
                let pid2 = growWeakThreads
                    ? ns.run("/hacking/weaken.js", growWeakThreads, target)
                    : pid1
                if (
                    ns.getRunningScript(pid1) == null ||
                    ns.getRunningScript(pid2) == null
                ...
                await ns.sleep(
                    Math.max(growTime, growWeakThreads ? growWeakTime : 0)
                )
                while (
                    ns.getRunningScript(pid1) != null ||
                    ns.getRunningScript(pid2) != null
            ...
            if (moneyCur == 0) {
                ...
                tempGrowThreads = Math.min(
                    Math.ceil(ns.growthAnalyze(target, moneyMax / moneyCur)),
                    10000
                )
            }
            let tempGrowTime = ns.getGrowTime(target)
            let tempGrowSecurity =
                ns.growthAnalyzeSecurity(tempGrowThreads, target, 1) +
                securityCur -
                securityMin
            let tempGrowWeakThreads
            for (
                tempGrowWeakThreads = 0;
                ns.weakenAnalyze(tempGrowWeakThreads) <= tempGrowSecurity;
                ++tempGrowWeakThreads
            );
            ...
            let pid1 = ns.run(
                "/hacking/grow.js",
                tempGrowThreads > 0 ? tempGrowThreads : 1,
                target
            )
            ns.print("RUN WEAK AFTER BUG")
            let pid2 = tempGrowWeakThreads
                ? ns.run("/hacking/weaken.js", tempGrowWeakThreads, target)
                : pid1
            if (
                ns.getRunningScript(pid1) == null ||
                ns.getRunningScript(pid2) == null
            ...
            await ns.sleep(
                Math.max(
                    tempGrowTime,
                    tempGrowWeakThreads ? tempGrowWeakTime : 0
                )
            )
            while (
                ns.getRunningScript(pid1) != null ||
    ...
    let instanceMemory =
        (2.6 * (2 * hackWeakTime - hackTime - growTime) +
            1.7 * (hackTime * hackThreads) +
            1.75 * (growTime * growThreads)) /
            hackWeakTime +
        1.75 * (hackWeakThreads + growWeakThreads)

    let availableMemory =
        ns.getServerMaxRam(ns.getHostname()) -
        ns.getServerUsedRam(ns.getHostname()) * 0.8
...
export async function debugPrint(ns, sleepOffset, target) {
    await ns.sleep(sleepOffset * 0.5)
    let moneyMax = ns.getServerMaxMoney(target)
    let securityMin = ns.getServerMinSecurityLevel(target)
    let moneyCur = ns.getServerMoneyAvailable(target)
    let securityCur = ns.getServerSecurityLevel(target)
    let message =
        ((moneyCur / moneyMax) * 100).toFixed(0).padStart(3) +
        " " +
        (securityCur - securityMin).toFixed(2) +
        "|"
    await ns.sleep(sleepOffset * 0.5)
