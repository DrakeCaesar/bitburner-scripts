...
export async function main(ns) {
  const target = ns.args[0],
        ...
        host = ns.getHostname();
  const growThreshold = 0.8, hackThreshold = 0.25, secTolerance = 0.01;
  ...
  async function runOp(script, threads, runtime, action) {
    ...
    const moneyPct = ((ns.getServerMoneyAvailable(target) / moneyMax) * 100).toFixed(2) + "%";
  ...
  async function weakenPhase() {
    while (ns.getServerSecurityLevel(target) > securityMin + secTolerance) {
      let excess = ns.getServerSecurityLevel(target) - securityMin, threads = 1;
      while (ns.weakenAnalyze(threads) < excess) threads++;
      const availableRam = ns.getServerMaxRam(host) - ns.getServerUsedRam(host),
            maxThreads = Math.floor(availableRam / ns.getScriptRam("/hacking/weaken.js"));
      threads = Math.min(threads, maxThreads);
      if (threads <= 0) { ns.print("Not enough RAM for weaken."); await ns.sleep(1000); continue; }
      await runOp("/hacking/weaken.js", threads, ns.getWeakenTime(target), "weaken");
    }
  }
  ...
  async function growPhase() {
    while (ns.getServerMoneyAvailable(target) < moneyMax * growThreshold) {
      ...
      const availableRam = ns.getServerMaxRam(host) - ns.getServerUsedRam(host),
            maxThreads = Math.floor(availableRam / ns.getScriptRam("/hacking/grow.js"));
      threads = Math.min(threads, maxThreads);
      if (threads <= 0) { ns.print("Not enough RAM for grow."); await ns.sleep(1000); continue; }
      
      // Debug: Print predicted percentages with threads and threads - 1.
      const predictedPct = Math.min(ns.formulas.hacking.growAmount(serverObj, player, threads, myCores) / moneyMax, 1) * 100,
            predictedPctLess = threads > 1 
              ? Math.min(ns.formulas.hacking.growAmount(serverObj, player, threads - 1, myCores) / moneyMax, 1) * 100 
              : predictedPct;
      ns.tprint(`DEBUG (grow): Using ${threads} threads -> predicted money: ${predictedPct.toFixed(2)}% of max.`);
      ns.tprint(`DEBUG (grow): Using ${threads - 1} threads -> predicted money: ${predictedPctLess.toFixed(2)}% of max.`);
  ...
  async function hackPhase() {
    while (ns.getServerMoneyAvailable(target) > moneyMax * hackThreshold) {
      const hackAmount = ns.getServerMoneyAvailable(target) - moneyMax * hackThreshold;
      let threads = Math.ceil(ns.hackAnalyzeThreads(target, hackAmount));
      const availableRam = ns.getServerMaxRam(host) - ns.getServerUsedRam(host),
            maxThreads = Math.floor(availableRam / ns.getScriptRam("/hacking/hack.js"));
      threads = Math.min(threads, maxThreads);
      if (threads <= 0) { ns.print("Not enough RAM for hack."); await ns.sleep(1000); continue; }
