...
export async function main(ns) {
  ...
  if (growThreads > 0) {
    ns.tprint(`Pre-Batch: Launching grow with ${growThreads} thread(s) to reach 100% money on ${target}.`);
    // 2. Exec grow.
    ns.exec("/hacking/grow.js", host, growThreads, target, 0);
  } else {
    ns.tprint(`Pre-Batch: Grow not needed because ${target} is already at 100% money.`);
  }
  ...
  if (weakenThreads > 0) {
    ns.tprint(`Pre-Batch: Launching weaken with ${weakenThreads} thread(s) to reduce security to base (${baseSecurity}).`);
    ns.exec("/hacking/weaken.js", host, weakenThreads, target, 0);
  } else {
    ns.tprint(`Pre-Batch: Weaken not needed because security is already at base.`);
  }
  ...
  ns.tprint(`Pre-Batch: Waiting ${waitTime} ms for grow and weaken to finish...`);
  ...
  if (postMoney < moneyMax * moneyTolerance) {
    ns.tprint(`WARNING: Pre-Batch money not at target. Got ${postMoney} (target ${moneyMax}).`);
  }
  if (postSec > baseSecurity + secTolerance) {
    ns.tprint(`WARNING: Pre-Batch security not at base. Got ${postSec} (target ${baseSecurity}).`);
  }
  ns.tprint(`Pre-Batch complete: ${target} now has ${postMoney} money and ${postSec} security.`);
  ...
  while (true) {
    ...
    const sleepGrow = weakenTime - growTime - batchDelay + batchCounter * batchDelay * 4;
