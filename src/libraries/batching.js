...
export function batchingThreads(ns, node) {
    const hackP = 0.9
    const server = ns.getServer(node)
    const growThreads = ns.growthAnalyze(node, 1 / (1 - hackP))
    const weakenThreads = ns.weakenAnalyze()

    server.maxRam
    const maxRam = server.maxRam * 0.9
    const loopRam =
...
export function batchingParameters(ns, node) {
    let hack_time = ns.getHackTime(node)
    let grow_time = ns.getGrowTime(node)
    let weak_time = ns.getWeakenTime(node)
    let t0 = 250
...
export function calculateGrowGain(ns, host, threads = 1, cores = 1, opts = {}) {
    ...
    return (
        Math.min(moneyMax, rate * (moneyAvailable + threads)) - moneyAvailable
    )
}
